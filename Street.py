...
class ExampleName(QtWidgets.QMainWindow, modelStreet.Ui_MainWindow):
    currentItemRow = None
    ...
    def cellChangedTable(self, item):
        if self.updateFlag == 1:
            ...
            api.windowStreetUpdate(self.newItemText,id)
        else:
            return
    ...
    def Delete(self):
        api.windowStreetDelete(self.deleteIndex)
    def Update(self):
        self.updateFlag = 1
    def loadData(self,name = False):
        if name is not False:
            sql = api.windowStreetLoadTable(name)
            res = conn.execute(sql)
        else:
            sql = api.windowStreetLoadTable()
            res = conn.execute(sql)
