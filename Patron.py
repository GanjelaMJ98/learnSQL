...
class ExampleName(QtWidgets.QMainWindow, modelPatron.Ui_MainWindow):
    currentItemRow = None
    ...
    def cellChangedTable(self, item):
        if self.updateFlag == 1:
            ...
            api.windowPatronUpdate(self.newItemText,id)
        else:
            return
    ...
    def Delete(self):
        api.windowPatronDelete(self.deleteIndex)
    def Update(self):
        self.updateFlag = 1
    def loadData(self,name = False):
        if name is not False:
            sql = api.windowPatronLoadTable(name)
            res = conn.execute(sql)
        else:
            sql = api.windowPatronLoadTable()
            res = conn.execute(sql)
